$(function(){
    ...
    //初始化、
	    ...
	    uploader1.on('uploadSuccess',function (file, response) {
	        ...
	        $("#product_img b").attr("data",response.ShowUrl)
	    ...
	    uploader2.on('uploadSuccess',function (file, response) {
	        ...
	        $("#product_album_list .imgClear").on("click",function(){
		    	$(this).parent().remove();
	    	})
	    });
	    //3.我的首图
	    ...
	    uploader3.on('uploadSuccess',function (file, response) { 
	       ...
	       /* var b=document.createElement("b");
	    	...
	    	$("#fistPoto b").attr("data",response.ShowUrl);
	    	var clearBtn = document.createElement("div");
	        clearBtn.innerHTML = "×";
	        $(clearBtn).addClass("imgClear");
	        $(clearBtn).on("click",function(){
    ...
    $.ajax({
        ...
        success: function (objResult,textStatus) {
            if(objResult.Result == 1){
                ...
                //性别
                if(model.Sex == 1){
                	$("#sex div").eq(0).addClass("on");
                }else{
                	$("#sex div").eq(1).addClass("on");
                }
				$("#tel").val(model.Mobile);
				$("#cityChoice").val(model.ServeCity);
				$("#birthday").val(model.Birth);
				$("#introduce").val(model.Inaword);
				introduct = model.Introduce;
                //头像
                var b=document.createElement("b");
			    $("#product_img").append(b)
			    b.setAttribute("data",model.Head);
				$("#product_img>.webuploader-pick").css("background-image",'url('+model.Head+')');
				
				
				var album = model.Album.split(",");
				for(var i = 0; i < album.length;i++){
					var imgbox = document.createElement("div");
					$(imgbox).append('<img src="'+album[i] +'" alt="">');
					$("#product_album").before(imgbox);
					var clearBtn = document.createElement("div");
			        clearBtn.innerHTML = "×";
			        $(clearBtn).addClass("imgClear");
			        $(imgbox).append(clearBtn);
			        $("#product_album_list .imgClear").on("click",function(){
				    	$(this).parent().remove();
			    	})
				}
				...
				var b=document.createElement("b");
			    $("#fistPoto").append(b)
			    b.setAttribute("data",model.Index);
				$("#fistPoto>.webuploader-pick").css("background-image",'url('+model.Head+')');
            }else{
    ...
    function saveFn(){
        ...
        //发送ajax请求
        var strActionName="Member_UpdateSalesMan";
        var arrActionParam = {
        	TailorID:TailorID,
		    Name:name,//姓名
			Sex:sex,//性别：0女；1男
